...
# Prepare data
def dataReading(csvfile):
##    X = []
##    Y = []
    df = np.asarray(pd.read_csv(csvfile))
    max1 = 0
    max2 = 0
    for m in df:
        n = np.asarray(eval(m[1]))
        if max1<n[:,0].max(): max1=n[:,0].max()
...
##                Ytemp.append(0)
##        Y.append(Ytemp)
    X_train, X_test, y_train, y_test = train_test_split(np.asarray(X,dtype=object),
...
trainX, trainY, testX, testY  = dataReading('samples.csv')
# Train the network
